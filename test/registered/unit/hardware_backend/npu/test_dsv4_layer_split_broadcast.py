...
register_npu_ci(est_time=300, suite="per-commit-2-npu-a2")
...
def _run(rank: int, world: int, port: int):
    ...
    # The all-gather test case flips the pool onto the fallback read path
    # directly; production selects it purely by process-group backend.
    if os.environ.get("DSV4_LS_TEST_ALLGATHER") == "1":
        pool._use_broadcast = False
        pool._use_async = False
    ...
    # scratch, plans swa pages {0,1}, then both ranks read layer 0 — the SAME
    # layer, so the collectives pair up (one rank owns it, the other stages
    # it): the non-owner's scratch must hold the owner's bytes on exactly the
    # planned pages, with unplanned pages left at zero.
...
class TestLayerSplitDSV4Broadcast(CustomTestCase):
    ...
    def test_owner_read(self):
        # Default path: async owner broadcast of the selected active pages.
        self._spawn_world(PORT)

    def test_owner_read_sync_broadcast(self):
        # Same transfers issued synchronously (async reads off).
        with envs.SGLANG_DSV4_LS_ASYNC_READ.override(False):
            self._spawn_world(PORT + 2)

    def test_owner_read_chunked_allgather(self):
        # Fallback path: chunked all-gather, forced plus one page per chunk so
        # the per-chunk staging copy / owner-slot copy loop runs.
        os.environ["DSV4_LS_TEST_ALLGATHER"] = "1"
        try:
            with envs.SGLANG_DSV4_LS_CHUNK_BYTES.override(128 * 512 * 2):
                self._spawn_world(PORT + 1)
        finally:
            os.environ.pop("DSV4_LS_TEST_ALLGATHER", None)
