...
class LayerSplitNPUDeepSeekV4SingleKVPool(NPUDeepSeekV4SingleKVPool):
    ...
    def _num_pages_for(self, local_layer_idx: int) -> int:
        full_pages = (self.size + self.kernel_page_size + 1) // self.kernel_page_size
        return full_pages if self._layer_owned_fn(local_layer_idx) else 0

    def create_buffer(self, *, num_pages: int):
        # The NPU bf16 base derives the page count from self.size and ignores
        # the argument, so honor num_pages here to keep non-owned layers at
        # 0 rows (a narrowed view would keep the full storage alive).
...
class LayerSplitNPUDeepSeekV4IndexerPool(NPUDeepSeekV4IndexerPool):
    ...
    def _owned_pages(self, local_layer_idx: int) -> int:
        full_pages = (self.size + self._kernel_page_size + 1) // self._kernel_page_size
        return full_pages if self._layer_owned_fn(local_layer_idx) else 0
...
# Buffer families served through owner-broadcast scratch copies. ``index_k``
# and ``index_scale`` are one logical family (the c4 indexer) invalidated
# together by a compressor epilog write.
...
class LayerSplitDSV4NPUTokenToKVPool(DSV4NPUTokenToKVPool):
    ...
    def __init__(self, *args, layer_shard_rank: int, layer_shard_size: int, **kwargs):
        ...
        # Built lazily on the first _make_kv_pool call: the sub-pool factories
        # run inside super().__init__ and need the ownership plan, while the
        # plan itself needs layer_num / compression_ratios / the PP stage
        # range the base class computes before those calls.
    ...
    def _make_kv_pool(
        ...
        *,
        ...
        # and C128 has an independent physical page size (mirrors the NPU base).
    ...
    def _init_remote_buffers(self) -> None:
        # One scratch buffer per family, shaped like a full layer of that
        # family. Every rank allocates every family: a rank may own no layer of
        # a bucket (e.g. no c128 layer), yet still receives owner broadcasts
        # for it.
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            device = self.device
            kv_dim = self.qk_nope_head_dim + self.qk_rope_head_dim
            ...
            def scratch(sub_pool, page_size: int, rows: int, cols: int, dt: torch.dtype):
                pages = (sub_pool.size + page_size + 1) // page_size
                return torch.zeros(
                    pages, page_size, rows, cols, dtype=dt, device=device
                )
        ...
        # family -> (layer_id, remote buffer): the last materialized remote copy.
        self._remote_layer_cache = {family: (None, None) for family in _REMOTE_FAMILIES}

    def _broadcast_read(self, family: str, layer_id: int) -> torch.Tensor:
        """Materialize ``family``'s buffer for ``layer_id`` and return the
        buffer this rank reads.

        Every CP rank issues the owner broadcast for the same layers in the
        same order, so a rank that skipped the call would hang its peers; the
        owner passes its local buffer as the src payload (no copy) and keeps
        reading it directly. Cache state stays in sync across the group because
        invalidation rides the write calls, which every rank issues.
        """
        local = self._local_family_buffer(family, layer_id)
        cached_layer, cached_buf = self._remote_layer_cache[family]
        if local is not None:
            ...
            if cached_layer == layer_id:
                return cached_buf
            target = self._remote_buffers[family]

        cp_group = get_parallel().attn_cp_group
        if cp_group.world_size > 1:
            cp_group.broadcast(target, src=self._layer_owner_rank(layer_id))
        self._remote_layer_cache[family] = (layer_id, self._remote_buffers[family])
        return local if local is not None else self._remote_buffers[family]

    def _invalidate_family(self, families: Tuple[str, ...], layer_id: int) -> None:
        for family in families:
            if self._remote_layer_cache[family][0] == layer_id:
                self._remote_layer_cache[family] = (None, None)
    ...
    def get_contiguous_buf_infos(self) -> Tuple[List[int], List[int], List[int]]:
        """Main PD buffers addressed by the full KV page id (owned only).

        NPU layout: ``[c4 KV, index K, index scale]``, each section restricted
