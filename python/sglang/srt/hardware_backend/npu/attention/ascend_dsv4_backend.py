...
class C4IndexerAscendBackendMixin:
    ...
    def _forward_prepare(
        ...
        q = self._compute_q_npu(
            c4_indexer, q_lora, self._cp_local_positions(forward_batch)
        )
...
class DeepseekV4AscendAttnBackend(
    ...
    def prepare_dsv4_cp_metadata(self, forward_batch: ForwardBatch) -> None:
        ...
        # Resolve the strategy: the global one under CP-v2, else the legacy
        # dsa_prefill_cp_mode flags imply it.
        ...
        # Legacy round-robin metadata lacks per-rank lengths the gather needs;
        # rebuild it. Under CP-v2 the runner-built metadata is canonical.
        from sglang.srt.layers.attention.dsa.utils import (
            is_dsa_prefill_cp_round_robin_split,
        )
        from sglang.srt.layers.cp.utils import is_cp_v2_active

        if not is_cp_v2_active(forward_batch) and is_dsa_prefill_cp_round_robin_split():
