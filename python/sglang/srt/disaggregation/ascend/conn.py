...
class AscendKVManager(MooncakeKVManager):
    ...
    def _is_layer_split_kv_transfer(self) -> bool:
        """Whether this prefill sends a layer-sharded KV registration.

        Layer split (prefill-CP only) registers only the CP rank's owned
        layers, so the positional pp_size == 1 send path cannot be used.
        """
    ...
    def get_mla_kv_ptrs_with_pp(
        ...
        if mla_ratios:
            ...
            # NPU main KV layout: [C4 KV, index K, index scale]. Under layer
            # split the prefill registers only its owned layers, so the dst
            # sections are sliced down to the same owned ranges; draft-model
            # buffers appended after the sections exist on both peers and pair
            # positionally.
